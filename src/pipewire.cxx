...
std::string Pipewire::name() const { return m_name; }
...
void Pipewire::on_global_removed(void *data, uint32_t id) {
  ...
  if (auto client = reinterpret_cast<Pipewire *>(data); client) {
    client->m_mutex.lock();

    if (id <= client->m_nodes.size())
      client->m_nodes[id - 1] = false;
    client->m_mutex.unlock();
  }
}
...
void Pipewire::on_global_added(void *data, uint32_t id, uint32_t permissions,
